...
# Точно как в trt_config.py репозитория (0 = person)
LABELS = [
...
def preprocess(frame, size):
    # как в официальном onnx_inference_image.py
...
def postprocess(outputs, orig_h, orig_w, input_size, score_thresh):
    # официальный порядок: boxes, class_ids, scores
    boxes     = outputs[0][0]   # [N, 4]  ymin, xmin, ymax, xmax  (нормализованные 0..1)
    class_ids = outputs[1][0]
    scores    = outputs[2][0]

    # как в репозитории: сначала в пиксели входа, потом scale к оригиналу
...
def main():
    ...
    while True:
        ...
        prev = now
        cv2.putText(frame, f"FPS {fps:.1f}  thr={threshold:.2f}  n={len(dets)}",
                    (10, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)

        cv2.imshow("EfficientDet-Lite0", frame)

        # раз в ~1 сек печатаем топ сырых детекций
